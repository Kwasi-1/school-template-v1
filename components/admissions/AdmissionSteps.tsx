        ...
        <div className="max-w-full xl:max-w-[85%] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
            {steps.map((step, index) => (
              ...
              >
                {/* Step Number */}
                <div className="absolute -top-4 left-6 bg-[#800020] text-white w-10 h-10 rounded-full flex items-center justify-center text-lg font-semibold">
                  {step.step}
                </div>

                {/* Icon */}
                <div className="mt-4 mb-6">
                  <div className="w-16 h-16 bg-[#800020]/10 rounded-2xl flex items-center justify-center group-hover:bg-[#800020] transition-colors duration-300">
                    <Icon
                      icon={step.icon}
                ...
                <p className="text-[#737477] leading-relaxed text-sm">
                  {step.description}
                </p>

                {/* Connector Line (hidden on last item and mobile) */}
                {index < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-1/2 -right-4 w-8 border-t-2 border-dashed border-[#800020]/30" />
                )}
              </div>
            ))}
          </div>
